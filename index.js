...
//console.log('task 6:', addArtist(artists, 20, 'Your Name Here', 'Your Birth Year - current day', 'Web Design', 'Your Nationality Here', 'lorem ipsum'))
...
//   let paintArtists = [];
//   for (let i = 0; i < array.length; i++) {
//     if (array[i].paintings.includes(100)); {
//       paintArtists.push(array[i].name)
//     } 
...
function artistByCountry(array, nationality){
    let artistsByNationality = []
  for (let i = 0; i < array.length; i++) {
    if (array[i].nationality === nationality)
    artistsByNationality.push(array[i].name)
}
